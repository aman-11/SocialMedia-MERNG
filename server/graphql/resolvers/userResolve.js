...
const { validateRegisterInput, validateLogin } = require('../../utils/Validator');
...
module.exports = {
    ...
    Mutation: {
        ...
        async register(_, {registerInput : { username, password, confirmPassword, email }}, context, info){
            ...
            const { errorList, valid } = validateRegisterInput(username, password, confirmPassword, email);
            
            console.log('in the userr',errorList);
            if(!valid) {
                throw new UserInputError('Validator', { errorList });
            }
            //2. make sure user dosent exists
            //3. hash pass auth tokens done
